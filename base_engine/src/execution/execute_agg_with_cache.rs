...
use crate::{AggregationRequest, ProcessedBaseMeasure, CacheableAggregationRequest, CacheableComputeRequest};
...
pub(crate) fn _exec_agg_with_cache<DS: CacheableDataSet + ?Sized>(
    ...
    for (measure_name, agg_name, pbm) in processed_base_measures {
        ...
        match data.get_cache().get(&cacheable_compute_request) {
            ...
            _ => yet_to_calculate.push({
                (cacheable_compute_request, pbm)
            }),
        }
    }
    ...
    let _new_res = if !yet_to_calculate.is_empty() {
        ...
        let (processed_base_measures, cacheable_request_column_name): (Vec<ProcessedBaseMeasure>, Vec<(CacheableComputeRequest, String)>)
         = yet_to_calculate.into_iter()
        ...
        //EXECUTE
        let new_res = super::_exec_agg_base(data, req.filters(),
            &req.add_row,
                &req.overrides,
                &req.groupby,
                req.totals,
            processed_base_measures, streaming)?;

        // Now save each of new measures to cache
        cacheable_request_column_name.into_iter()
