...
pub fn execute(req: DataRequestS, data: &impl DataSet)
 -> Result<DataFrame>{ 
    ...
    // Unpack
    let (newnames, (aggregateions, fltrs)): (Vec<String>, (Vec<Expr>, Vec<Option<Expr>>)) = prepared_measures.into_iter()
    ...
    for fltr in fltrs {
        match fltr {
            Some(f) => {measure_filter = measure_filter.or(f);}
            None => {measure_filter = measure_filter.or(lit::<bool>(true));}
        }
    }
...
fn measure_builder(requested_measures: &[(String, String)], 
    all_availiable_measures: &MM,
    op: &OCP ) -> Vec<ProcessedMeasure>
{
    ...
    for (measure_name, action) in requested_measures {
        if let Some(m) = all_availiable_measures.get(measure_name as &str)  {
            ...
            } else {warn!("Aggregation action: {action} not found"); continue }
        } else {warn!("Measure: {measure_name} not found"); continue }
    }
    res
}
