...
mod agg_req_tests {
    ...
    fn simple_fltr_grpby_sum() {
        let req = r#"{"measures": [["Balance", "sum"]], "groupby": ["State"], "filters": [[{"op": "Eq", "field": "State", "value": "NY"}]]}"#;
        let data_req =
    ...
    //     //     let req =
    //     //         serde_json::from_str::<AggregationRequest>(raw_req).expect("Could not parse request");
