...
pub(crate) fn frtb_calc_params() -> Vec<CalcParameter> {
        ...
        res.push(CalcParameter{name: "jurisdiction".into(), default: Some("BCBS".into()), type_hint: Some("String".into())});
        res.push(CalcParameter{name: "drc_offset".into(), default: Some("true".into()), type_hint: Some("bool".into())});
        ...
        for (name, default, type_hint) in params {
            res.push(CalcParameter{name: format!("{}_high", name), default: default.clone(), type_hint: type_hint.clone()});
            res.push(CalcParameter{name: format!("{}_medium", name), default: default.clone(), type_hint: type_hint.clone()});
            res.push(CalcParameter{name: format!("{}_low", name), default, type_hint});
        }
