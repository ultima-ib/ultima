...
use base_engine::{LazyFrame, Series, NamedFrom, PolarsResult, DataType, 
    col, concat, IntoLazy, JoinType, Utf8NameSpaceImpl, IntoSeries,
    GetOutput};

use crate::{prelude::{frame_from_path_or_str, rcat_rc_b_weights_frame}, 
            drc::drc_weights};
...
pub fn weights_assign_crr2(lf: LazyFrame, build_params: &HashMap<String, String>) -> PolarsResult<LazyFrame> {
    let check_columns = [col("RiskClass").cast(DataType::Utf8), col("RiskCategory").cast(DataType::Utf8), col("BucketCRR2").cast(DataType::Utf8), col("WeightsCRR2").cast(DataType::Float64)];
    let check_columns4 = [col("RiskClass").cast(DataType::Utf8), col("RiskCategory").cast(DataType::Utf8), col("CreditQuality").cast(DataType::Utf8), col("WeightsCRR2").cast(DataType::Float64)];
...
pub fn weight_assign_logic_crr2(lf: LazyFrame, weights: SensWeightsConfig) -> PolarsResult<LazyFrame> {

    // First, left join one by one
    let join_on = [col("RiskClass"), col("RiskCategory"), col("BucketCRR2")];
    let mut lf1 = lf.join(weights.rc_rcat_b_weights_crr2, join_on.clone(), join_on, JoinType::Left);
    ...
    let mut lf1 = lf1.join(weights.drc_nonsec_weights_frame_crr2, join_on.clone(), join_on, JoinType::Left);
    lf1 = lf1.with_column(col("SensWeightsCRR2").fill_null(col("WeightsCRR2")))
