...
pub(crate) fn com_total_low(op: &CPM) -> PolarsResult<Expr> {
    commodity_delta_charge_low(op) + com_vega_charge_low(op) + com_curvature_charge_low(op)
}
pub(crate) fn com_total_medium(op: &CPM) -> PolarsResult<Expr> {
    commodity_delta_charge_medium(op) + com_vega_charge_medium(op) + com_curvature_charge_medium(op)
}
pub(crate) fn com_total_high(op: &CPM) -> PolarsResult<Expr> {
    commodity_delta_charge_high(op) + com_vega_charge_high(op) + com_curvature_charge_high(op)
}
