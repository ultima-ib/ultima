...
pub(crate) fn eq_total_low(op: &CPM) -> PolarsResult<Expr> {
    equity_delta_charge_low(op) + equity_vega_charge_low(op) + eq_curvature_charge_low(op)
}
pub(crate) fn eq_total_medium(op: &CPM) -> PolarsResult<Expr> {
    equity_delta_charge_medium(op) + equity_vega_charge_medium(op) + eq_curvature_charge_medium(op)
}
pub(crate) fn eq_total_high(op: &CPM) -> PolarsResult<Expr> {
    equity_delta_charge_high(op) + equity_vega_charge_high(op) + eq_curvature_charge_high(op)
}
