...
/// Returns a Series equal to SensitivitySpot with RiskClass == FX and RiskFactor == ...CCY where CCY is Reporting CCY
/// takes CalcParams because we need to know reporting CCY, USD by default
pub(crate) fn fx_delta_sens (op: &OCP) -> Expr {
    let rep_ccy_regex = op.as_ref()
