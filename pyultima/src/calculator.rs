...
use ultibi::polars::{prelude::DataType, lazy::dsl::{col, apply_multiple}};
use pyo3::{pyclass, pymethods, types::{PyType, PyList, IntoPyDict}, PyResult, PyObject, Python};
use ultibi::{Calculator, CPM, PolarsResult};
use crate::conversions::{wrappers::Wrap, series::{rust_series_to_py_series, py_series_to_rust_series}};
...
impl CalculatorWrapper {
    ...
    pub fn standard(_: &PyType, lambda: PyObject) -> PyResult<Self> {
        let calculator = move |op: &CPM| {
            ...
            Python::with_gil(move |py| {
                ...
                let out = lambda.call(py, (op.into_py_dict(py),), None)
    ...
    pub fn custom(_: &PyType,
        lambda: PyObject,
        output_type: Wrap<DataType>,
        inputs: Vec<String>, 
        returns_scalar: bool) -> PyResult<Self> {
...
pub(crate) fn call_lambda_with_args_and_series_slice(
    ...
    lambda.call(py, (wrapped_s, kwargs.into_py_dict(py)), None)
        .map_err(|e|PolarsError::ComputeError(e.value(py).to_string().into()))
