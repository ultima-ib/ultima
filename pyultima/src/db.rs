...
impl DbInfo {
    ...
    fn __init__(table: String, db_type: String, conn_uri: String, schema: Option<Vec<(String, Wrap<DataType>)>>) -> Self {
        let schema = schema.map(|field| 
            Schema::from_iter(
                field.into_iter().map(|(name, wrap)|Field::new(name.as_str(), wrap.0))
            )
        )
        .map(Arc::new);

        DbInfo { inner: DbInfoInner {table, db_type, conn_uri, schema} }
    }
